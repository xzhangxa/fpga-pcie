...
uint64_t intel_fpga_pcie_dev::allocate_buffer(void)
{
     uint64_t address;
    //std::cout << "CALL ALLOCATE BUFFER "  << std::endl;
    //std::cout << "address " << address  << std::endl;
    int result = ioctl(m_dev_handle, INTEL_FPGA_PCIE_IOCTL_ALLOCATE_BUFFER, &address);
// HSD:15010575718 warning clean up - wrote a dump if statement to use ‘result’ to clean up unused variable warning, cannot remove the line, else will have unintialized warning for address    
    if (result != 0) {
    }
    //std::cout << "address " << address  << std::endl;
    return address;
...
/*unsigned int intel_fpga_pcie_dev::release_buffer(void)
{
    int result = ioctl(m_dev_handle, INTEL_FPGA_PCIE_IOCTL_ALLOCATE_BUFFER, 0x1U);
    return result;
}*/